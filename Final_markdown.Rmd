...
pacman::p_load(tidyverse, vroom, DataExplorer, lubridate, sf, tidycensus, tigris, hms, riem, slider, scales, caret,RcppEigen, ckanr, FNN, ggcorplot, grid, gridExtra, gtools, httr, kableExtra, modelsummary,tidycensus, spdep, stargazer, tmap, viridis, zipcodeR, DT)
...
Sample <- df %>%
          select(!remove)
Sample <- Sample[sample(nrow(Sample), size = 1000000), ]
...
Test <- bind_cols(Test, f) %>%
          rename(Delay.Predict = ...18) %>%
...
Test$from <- str_replace(Test$from, Station_clean$from[i],Station_clean$clean_name[i])
}

for(i in 1:nrow(Station_clean)) {
Test$to <- str_replace(Test$to, Station_clean$from[i],Station_clean$clean_name[i])
}
